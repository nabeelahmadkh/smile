...
class ImagePreviewVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
    ...
    override func viewDidLoad() {
        ...
        print("the index of the IMAGE IS \(passedContentOffset)")
        temp = passedContentOffset[1]
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath[1] != 0{
            if ((indexPath.row + temp) < (imgArray.count)){
                cell.imgView.image=imgArray[indexPath.row + temp]
            }
            else{
                if i > (imgArray.count - 1){
                    i = 0
                }
                cell.imgView.image = imgArray[i]
                i = i+1
            }
        }
        else{
            cell.imgView.image=imgArray[temp]
        }
        print("THE INDEX PATH IS \(indexPath) \(temp)")
        return cell
    }
